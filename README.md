...
8. 保存后，在任何 App 里复制链接 → 分享按钮 → 点更多 → 下滑找到「一键收录」（可将快捷方式收藏让位置靠前）；小红书因平台限制，需先复制链接到微信或浏览器后才能分享

> ⚠️ **手机端收录超时怎么办？** 如果快捷指令运行很久后提示「请求超时」，通常是网络问题——`workers.dev` 域名在国内部分网络下会被墙或不稳定。解决办法：① 开启代理/VPN 后重试；② 或给 Worker 绑定自己的自定义域名（如 `kb.你的域名.com`），把快捷指令 URL 里的域名替换掉。域名需是国内可正常访问的域名，可在阿里云/腾讯云/万网等平台购买（绑定方法上网搜「Cloudflare Worker 绑定自定义域名」教程）。电脑端 Chrome 插件遇到同样问题也是这个原因。
