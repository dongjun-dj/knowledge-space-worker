...
> ⚠️ **手机端收录超时怎么办？** 如果快捷指令运行很久后提示「请求超时」，通常是网络问题——`workers.dev` 域名在国内部分网络下会被墙或不稳定。解决办法：① 开启代理/VPN 后重试；② 或给 Worker 绑定自己的自定义域名（如 `kb.你的域名.com`），把快捷指令 URL 里的域名替换掉。电脑端 Chrome 插件遇到同样问题也是这个原因。
